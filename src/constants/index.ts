export const COMPANY = {
  name: "Shwapnopuri Properties & Developments",
  shortName: "Shwapnopuri",
  phone: process.env.NEXT_PUBLIC_PHONE || "+8801342642413",
  email: process.env.NEXT_PUBLIC_EMAIL || "shwapnopuridevelopments@gmail.com",
  address: "Pashchim gaon, Mia Para, P.O. Pashchim gaon, Post code 3570, Laksam, Cumilla, Bangladesh",
  facebook: "https://facebook.com",
  instagram: "https://instagram.com",
  youtube: "https://youtube.com",
};

export const NAV_LINKS = [
  { label: "Home", href: "/" },
  { label: "About", href: "/about" },
  { label: "Documents", href: "/documents" },
  { label: "Gallery", href: "/gallery" },
];


export const SKILL_OPTIONS = [
  "CONSTRUCTION",
  "ELECTRICAL ENGINEER",
  "ELECTRICIAN",
  "PLUMBING",
  "CARPENTRY",
  "MASONRY",
  "PAINTING",
  "WELDING",
  "DRIVING",
  "TAILORING",
  "COMPUTER",

  "SALES EXECUTIVE",
  "MARKETING EXECUTIVE",
  "DIGITAL MARKETING EXECUTIVE",
  "HEAD OF SALES",

  "ARCHITECT / SENIOR ARCHITECT",
  "PROJECT ENGINEER / SITE ENGINEER",
  "ASSISTANT / DEPUTY PROJECT ENGINEER",
  "INTERIOR ENGINEER",
  "ESTIMATION ENGINEER",

  "MASON",
  "ROD BINDING / SCAFFOLDING",
  "PLUMBER",
  "ELECTRICAL",

  "WAREHOUSE KEEPER",
  "CLEANER WORKER",

  "CIVIL ENGINEER",
  "SECURITY GUARD",
];







  // "Construction",
  // "Electrical",
  // "Plumbing",
  // "Carpentry",
  // "Masonry",
  // "Painting",
  // "Welding",
  // "Driving",
  // "Cooking",
  // "Tailoring",
  // "Computer",



//CONSTRUCTION
//ELECTRIC ENGINEER 
//ELECTRICIAN
//PLUMBING
//CARPENTRY
//MASONRY
//PAINTING
//WELDING
//DRIVING
//COOKING
//TAILORING
//COMPUTER




  // "Sales Executive",
  // "Marketing Executive",
  // "Degital Marketing Executive",
  // "Head of sales manager",

  // "Architect /Senior Architect",
  // "Project Engineer/ site Engineer",
  // "Assistant / Deputy Prezeler Engineer",
  // "Interior Engineer",
  // "Estimation Engineer",




// SALES EXECUTIVE
// MARKETING EXECUTIVE
// DIGITAL MARKETING EXECUTIVE
// HEAD OF SALES MANAGER
// ARCHITECT /SENIOR ARCHITECT
// PROJECT ENGINEER/ SITE ENGINEER
// ASSISTANT / DEPUTY PREZELER ENGINEER
// INTERIOR ENGINEER
// ESTIMATION ENGINEER







    //  "Massion",
    //   "Rod binding / Scaffolding",
    //   "Plumber",
    //   "Electrical",
    //   "WAREHOUSE-KERPER",
    //   "CLEANER WORKER", 



// MASON
// ROD BINDING / SCAFFOLDING 
// PLUMBER 
// ELECTRICAL
// WAREHOUSEKERPER
// CLEANER WORKER
// CIVIL ENGINEER
// SECURITY GUARD















































// Sales and Marketing 
// 1. Sales Executive 
// 2. Marketing Executive 
// 3. Degital Marketing Executive. 
// 4. Head of sales manager 


//  Engineering and project 
// 5.Architect /Senior Architect 
// 6.Project Engineer/ site Engineer
// 7.Assistance /Deputy প্রিজেলার Engineer
// 8.Interior Engineer
// 9.Estimation Engineer


// Labour Department 
// 10. Massion
// 11. Rod binding / Scaffolding 
// 12. Plumber 
// 13. Electrical
// 14. WAREHOUSE-KERPER.
// 15. CLEANER WORKER